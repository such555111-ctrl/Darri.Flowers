...
log = logging.getLogger("darri_flowers")
...
def cmd_start(message: types.Message):
    if not BASE_URL:
        bot.send_message(
            message.chat.id,
            "Darri.Flowers: мини-апп ещё не сконфигурирован (не задан WEBHOOK_URL)."
        )
        return
    ...
    bot.send_message(
        message.chat.id,
        "🌸 *Darri.Flowers — Цветы. Доставка. Чебоксары*\n\n"
        "Добро пожаловать в Darri.Flowers! Букеты, розы, тюльпаны, композиции "
        "и авторская флористика с доставкой по городу.\n\n"
...
def customer_cancel_order(oid):
    ...
    for admin_id in ADMIN_CHAT_IDS:
        try:
            bot.send_message(admin_id, f"❌ Клиент отменил заказ DF-{oid}")
        except Exception as e:
            log.warning("Не удалось уведомить администратора %s: %s", admin_id, e)
...
def admin_message_customer(oid):
    ...
    try:
        bot.send_message(
            chat_id,
            f"💬 Сообщение от Darri.Flowers по заказу DF-{oid}:\n\n{text}"
        )
    except Exception as e:
...
def admin_message_question(qid):
    ...
    try:
        bot.send_message(
            chat_id,
            f"💬 Ответ от Darri.Flowers по товару «{q.get('product_name','')}»:\n\n{text}"
        )
    except Exception as e:
